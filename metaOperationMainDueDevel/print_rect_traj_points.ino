void print_rect_traj_points()
{
  //float * q2print;
  
  DEBUG_SERIAL.println(F("[    INFO    ] TRAJECTORY SELECTED "));
  DEBUG_SERIAL.println(F("[ CNT ]  [ JOINT1 ]  [ JOINT2 ]  [ JOINT3 ] "));

  for (size_t i = 0; i < MAX_TRAJ_POINTS1; i++) // FOR ALL POINTS
  {
    ...
    for (size_t j = 0; j < TOTAL_ACTIVE_JOINTS; j++) // FOR EACH ACTIVE JOINT
    {
       ...
       DEBUG_SERIAL.print(ptr2traj_struct->traj_cs_points[i][j],4); DEBUG_SERIAL.print(F("   "));
    }
