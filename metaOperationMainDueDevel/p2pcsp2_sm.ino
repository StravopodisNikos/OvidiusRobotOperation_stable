  ...
  if (return_function_state)
  {
    DEBUG_SERIAL.println(F("[  INFO  ] STEPPER VELOCITY PROFILE STEPS [  SUCCESS ]"));
    DEBUG_SERIAL.println(F("[====================================================]"));
   ...
   */
   for(size_t id_count = 0; id_count < sizeof(dxl_id); id_count++)
   {
      ...
      // [26-8-21] Modify velocity for (yet) undefined bug
      /*
      if(dxl_prof_vel[id_count] < MIN_DXL_PV )
      {
        dxl_prof_vel[id_count] = MIN_DXL_PV;
      }
      */
  ...
  // 6: var time step for given Velocity [31-12-21]
...
*/
  ...
  DEBUG_SERIAL.print(F("[    INFO    ]")); DEBUG_SERIAL.println(F(" P2P EXECUTION "));
