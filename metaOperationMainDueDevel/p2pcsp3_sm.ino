void p2pcsp3_sm(float * q_i, float * q_f, float Texec)
{
  /*
   * This function is based on p2pcsp2 BUT ONLY used for 
   * executing a sequence of points trajectory.
   * 
   * q_f -> [ goalAbsPos_double,Qdxl_f[3] ] is a 4x1 array
   * q_i -> [ currentAbsPos_double,Qdxl_i[3] ] is a 4x1 array
   * Author N. Stravopodis
   * 
   * [31-12-21] Modified to use Real time Velocity Ramp and smooth
   *            transition between comsecutive waypoints.
   */
   
  //DEBUG_SERIAL.println(F("[  INFO  ] NEXT C-SPACE POINT... "));
  ...
  */
  ...
  // [29-7-21] IMPLEMENTS LOGGING - MUST BE DONE BEFORE SYNC SET GOAL POS TO DYNAMIXELS!
  //set_sensor_updates(); -> WILL BE PLACED BEFORE TRAJ EXECUTION STARTS!
  ...
  // SELECT EXECUTE FUNCTION 5 OR 6
  // 5: fixed time step for given Velocity
  return_function_state =  stp.syncSetStepperGoalPositionVarStep5(PTR2RobotDataLog, SENSOR_LOG_STRUCTS, ForceSensor, ForceSensorHX711, ptr2joint1_cur_sensor, &sensor_error, finalTargetSet, &currentAbsPos_double, &goalAbsPos_double, q_f, &Vexec, &LIN_SEG_EXISTS, update_sensor_measurements , P2P_PROF_STEPS,  PTR_2_meta_dxl, PTR_2_dxl_pp_packet, PTR_2_dxl_pv_packet, PTR_2_dxl_pc_packet, PTR_2_dxl_mov_packet,  &error_code_received);  
  // 6: var time step for given Velocity [31-1-21]
  //return_function_state =  stp.syncSetStepperGoalPositionVarStep6(PTR2RobotDataLog, SENSOR_LOG_STRUCTS, ForceSensor, ForceSensorHX711, ptr2joint1_cur_sensor, &sensor_error, finalTargetSet, &currentAbsPos_double, &goalAbsPos_double, q_f, p2pcsp_Ta, Texec, Aexec, Vexec, &LIN_SEG_EXISTS, update_sensor_measurements , P2P_PROF_STEPS,  PTR_2_meta_dxl, PTR_2_dxl_pp_packet, PTR_2_dxl_pv_packet, PTR_2_dxl_pc_packet, PTR_2_dxl_mov_packet,  &error_code_received);  
  // 7: var time step for given Velocity & smooth waypoint transition [1-1-22]
  //return_function_state =  stp.syncSetStepperGoalPositionVarStep7(PTR2RobotDataLog, SENSOR_LOG_STRUCTS, ForceSensor, ForceSensorHX711, ptr2joint1_cur_sensor, &sensor_error, finalTargetSet, &currentAbsPos_double, &goalAbsPos_double, q_f, p2pcsp_Ta, Texec, Aexec, Vexec, &LIN_SEG_EXISTS, update_sensor_measurements , P2P_PROF_STEPS,  PTR_2_meta_dxl, PTR_2_dxl_pp_packet, PTR_2_dxl_pv_packet, PTR_2_dxl_pc_packet, PTR_2_dxl_mov_packet,  &error_code_received);  
  ...
  if (return_function_state){
    ...
    // [9-4-21] IF syncSet5 is used here:
    // 1. logged data are printed in serial monitor
    // 2. final write to the log file must take place
    // print_logged_data();
  }
  else
  {
  ...
  */
  ...
  //DEBUG_SERIAL.print(F("[    INFO    ]")); DEBUG_SERIAL.println(F(" SEGMENT EXECUTION FINISHED "));
