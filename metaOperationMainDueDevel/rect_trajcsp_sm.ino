void rect_trajcsp_sm()
{
  ...
  p2pcsp2_sm(currentConfiguration, ptr2traj_struct->traj_cs_points[0], ptr2traj_struct->time_intervals[0]);
  ...
  for (size_t i = 0; i < (MAX_TRAJ_POINTS1-1); i++)
  {
    ...
    Qf = ptr2traj_struct->traj_cs_points[pair_cnt+1];
    Tseg = ptr2traj_struct->time_intervals[pair_cnt+1];

    if ( i == (MAX_TRAJ_POINTS1-2) )
    {
      finalTargetSet = true;
    }
  ...
  DEBUG_SERIAL.print(F("[    INFO    ]")); DEBUG_SERIAL.println(F(" RECT-TRAJECTORY EXECUTED "));
